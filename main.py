...
class Player:
    def __init__(self, id, money=2000, stepped_on=go):
        self.id = id
...
def player_check(player, event, throws):
    ...
    if player.is_in_jail:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pay_jail_button.is_hovering(mouse_pos):
                player.money -= 50
                player.is_in_jail = False
...
def graphics(gamestart, player_list, rolled):
    ...
    # Изобрзаява бутоните
    quit_button.show_button(screen, darker_red, calibri)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if gamestart:
                ...
                if not rolled:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if dice_button.is_hovering(mouse_pos):
                            rolled = True
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if end_turn_button.is_hovering(mouse_pos):
                            current_player += 1
                            rolled = False
